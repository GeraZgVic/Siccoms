                    ...
                    <div class="group h-96 w-96 perspective-1000">
                        <div class="relative h-full w-full rounded-xl shadow-xl transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                            <div class="absolute inset-0 rounded-xl overflow-hidden">
                                <picture>
                                    @foreach ($servicio['imagen_alternativas'] as $extension => $ruta)
                                        <source srcset="{{ asset($ruta) }}" type="image/{{ $extension }}">
                                    @endforeach
                                    {{-- IMAGEN PRINCIPAL FORMATO JPG --}}
                                    <img src="{{ asset($servicio['imagen']) }}" alt="{{ $servicio['titulo'] }}" loading="lazy" class="object-cover w-full h-full">
                                </picture>
                            </div>
